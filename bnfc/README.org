...
to enable gf-only ouput, run =stack run -- l4 gf en < l4/test.l4 > out/test.out=

You can choose a specific language by using the =en= (default) or =my= (future) modifier.

** To get Dot output

to enable dot-only output, run =stack run -- l4 dot < l4/test.l4 > out/test.out=
