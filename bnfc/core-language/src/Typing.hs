...
super_classes_decls cds =
  ...
  in map (super_classes cdf_assoc [] . fst) cdf_assoc
...
wellformed_class_decls_in_module md =
  case md of
    (Mdl cds rls) ->
      let class_names = map classDeclName cds
      in all (defined_superclass class_names) cds && not (hasDuplicates (map classDeclName cds))
...
strict_superclasses_of md cn = case lookup cn (class_def_assoc (moduleClassDecls md)) of
  Nothing -> error ("in strict_superclasses_of: undefined class " ++ clsNmStr cn)
...
lookup_class_def_in_env env cn =
  map classDeclDef (filter (\cd -> classDeclName cd == cn) (moduleClassDecls (module_of_env env)))
...
tp_constval env x = case x of
  ...
  RecordV cn fnvals ->
    ...
    in case lookup_class_def_in_env env cn of
       [] -> error ("class name " ++ clsNmStr cn ++ " not defined")
       [cd] ->
         if fields_of_class_def cd == tfnvals
         then ClassT cn
         else error ("record fields do not correspond to fields of class " ++ clsNmStr cn)
...
list_subset xs ys = all (`elem` ys) xs
