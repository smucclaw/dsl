...
Given only those types, and the source text above, we construct an AST. Subsequently we will write a parser that turns the L4 syntax above into this AST.

For now we take the eDSL approach with a bit of syntactic sugar, and write the AST from scratch.
