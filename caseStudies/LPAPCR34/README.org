...
Given only those types, and the structure above, we construct an AST. Subsequently we will write a parser that turns the L4 syntax into this AST. For now we take the eDSL approach with a bit of syntactic sugar, and write the AST from scratch.
...
**** A simple app prints the manually defined
...
#+begin_src haskell :noweb-ref 34-prolog
  ...
  stmTorolog (DRule rn p d a w) = unlines [ unwords [ "%    RULE", teToProlog rn ],
