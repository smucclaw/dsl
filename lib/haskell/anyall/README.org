...
After re-reading [[https://drive.google.com/file/d/1FXHyxDhjQJ5plDC_a7yCPUrxIu8EBhYp/view?usp=sharing][Layman Allen]] and considering the [[https://en.wikipedia.org/wiki/Ladder_logic][ladder logic used in PLC controllers]], we tried a new design.
...
If the user has not given a truth value to the proposition, it is in an "unknown" state -- ~Maybe Bool~.
...
The question is phrased as ~Not (Leaf "you are a public agency")~ to allow a more natural interview with the end-user.

We draw a solid line at the end of the box.
...
If the user answers "yes" to the question the overall value becomes "no", and vice-versa.
