...
docQ1 m (Node (Q sv  Neg                           v) c) = markbox v sv <+>              ": !" <+> nest 2 (hsep $ docQ1 m <$> c)
docQ1 m (Node (Q sv (Simply a)                     v) _) = markbox v sv <+> pretty a
docQ1 m (Node (Q sv  And       v) c) = markbox v sv <> nest 2 (ppline <> vsep ((\i -> "&" <+> docQ1 m i) <$> c))
docQ1 m (Node (Q sv  Or        v) c) = markbox v sv <> nest 2 (ppline <> vsep ((\i -> "|" <+> docQ1 m i) <$> c))
