...
-- Variations on root_nsubj_*
    ...
    {- The lincat of advmod is {adv : Adv ; isNot : Bool} (from UDCatMay.gf)
       The two constructors for advmod are: (found in UDCat.gf)
         1) advmod_    : Adv -> advmod
         2) not_advmod : advmod
       They create the following kinds of advmods: (found in UDCatMay.gf)
    ...
    root_nsubj_advmod rt sub advm =
      ...
      case advm.isNot of {
        ...
        False => -- the advmod comes from advmod_ : Adv -> advmod
          ...
          -- because obl's lincat is Adv (found in UDCatMay.gf)
          -- and that's the same type that is in advmod's adv field (also in UDCatMay.gf)
      } ;
...
-- Needed to add special funs that are not the top-level/application layer
  ...
  oper
    ...
    addMark : Subj -> LinUDS -> LinUDS = \if,breachOccurs ->
      let if_Predet : Predet = lin Predet if ; -- hack: Predet and Subj have the same lincat in Maylish RG
...
{-
  applyAux : CatMay.Pol -> LinAux -> VP -> UDSPred = \pol,will,sleep ->
    case will.auxType of {
  ...
  applyAux2 : CatMay.Pol -> (may, have : LinAux) -> VP -> UDSPred = \pol,may,have,sleep ->
