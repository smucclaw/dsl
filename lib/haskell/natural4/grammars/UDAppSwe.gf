...
concrete UDAppSwe of UDApp =
  UDCatSwe, JustWordsWordNetSwe - [NP, some_Quant, some_Det, any_Det] **
  open Prelude, SyntaxSwe, IrregSwe, ExtendSwe,
    SymbolicSwe,
    (CS=CommonScand)
    in {
...
-- Variations on root_nsubj_*
    ...
    {- The lincat of advmod is {adv : Adv ; isNot : Bool} (from UDCatSwe.gf)
       The two constructors for advmod are: (found in UDCat.gf)
         1) advmod_    : Adv -> advmod
         2) not_advmod : advmod
       They create the following kinds of advmods: (found in UDCatSwe.gf)
    ...
    root_nsubj_advmod rt sub advm =
      ...
      case advm.isNot of {
        ...
        False => -- the advmod comes from advmod_ : Adv -> advmod
          ...
          -- because obl's lincat is Adv (found in UDCatSwe.gf)
          -- and that's the same type that is in advmod's adv field (also in UDCatSwe.gf)
      } ;
...
-- Variations on root_obj_*
    ...
    root_obj_nmod processes personal_data for_purposes =
      root_obj processes (np2nplite (mkNP <lin NP (nplite2np personal_data) : NP> for_purposes)) ;
...
{-
    ...
    addMark : Subj -> LinUDS -> LinUDS = \if,breachOccurs ->
      let if_Predet : Predet = lin Predet {s = \\_,_ => if.s ; p = [] ; a = CS.PNoAg} ;
...
-}
    ...
    onlyPred : VP -> LinUDS = \vp ->
      let uds : LinUDS = mkUDS (np2nplite it_NP) (mkRoot vp)
       in uds ** {hasSubj = False} ;
    onlyPred : LinRoot -> LinUDS = \rt ->
      let uds : LinUDS = mkUDS (np2nplite it_NP) rt
       in uds ** {hasSubj = False} ;
    onlyPred : UDSPred -> LinUDS = \pr ->
      let uds : LinUDS = mkUDS (np2nplite it_NP) pr
       in uds ** {hasSubj = False}
    } ;

  np2nplite : CatSwe.NP -> NPLite = \np -> np ;
...
{-
  applyAux : CatSwe.Pol -> LinAux -> VP -> UDSPred = \pol,will,sleep ->
    case will.auxType of {
  ...
  applyAux2 : CatSwe.Pol -> (may, have : LinAux) -> VP -> UDSPred = \pol,may,have,sleep ->
