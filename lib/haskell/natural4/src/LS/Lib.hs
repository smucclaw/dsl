...
import LS.Rule
    ( defaultHorn,
      ...
      Expect(ExpRP),
      Parser,
      Rule(NotARule, RuleGroup, Scenario, RegFulfilled, RegBreach,
           Hornlike, TypeDecl, Constitutive, Regulative, RuleAlias, name,
           scgiven, expect, clauses, giveth, enums, has, super, letbind,
           keyword, subj, rkeyword, who, cond, deontic, action, temporal,
           hence, lest, rlabel, lsource, srcref, upon, given, having, wwhere,
           defaults, symtab),
      RuleBody(..) )
import LS.Tokens
    ( asks,
