...
isMtexprText :: MTExpr -> Maybe Text.Text
isMtexprText (MTT t) = Just t
isMtexprText _ = Nothing
...
data SimpleHlike a b c =
  MkSimpleHL { shGiven :: a
             -- ^ The 'GIVEN' part of a horn-like rule.
             , shReturnVariables :: b
             -- ^ The 'GIVETH' part of a horn-like rule.
             , shSrcRef :: SrcRef
             -- ^ Source reference to where the rule was defined.
             -- While there is 'SrcRef' is usually generated by the
             -- csv file parser, there is nothing that ties it inherently into
             -- the csv pipeline.
             --
             -- TODO: Perhaps we want to be able to parameterise the type of
             -- source reference we might have. For example, to have Source Spans
             -- instead of only Source References.
             , shClauses :: BaseHL
             -- ^ Clauses of the horn-like rule. Contains at least one clause.
             , shLabel :: c
...
newtype BaseHL = BaseHL (NonEmpty AtomicHC)
  deriving stock (Generic)
  deriving newtype (Eq, Show)

{-# COMPLETE OneClause, MultiClause #-}

-- | 'BaseHL' consists of exactly one clause.
pattern OneClause :: AtomicHC -> BaseHL
pattern OneClause x <- BaseHL (x :| []) where
  OneClause x = BaseHL (x :| [])

-- | 'BaseHL' consists of at least two clauses.
pattern MultiClause :: NonEmpty AtomicHC -> BaseHL
pattern MultiClause xs <- BaseHL xs@(_ :| _:_) where
  MultiClause xs = BaseHL xs

data HornBodyHeadClause =
  MkHornBodyHeadClause
    { hbHead :: RelationalPredicate
    , hbBody :: BoolStructR
    }
  deriving stock (Eq, Show, Generic)

data AtomicHC = HeadOnly HeadOnlyHC
              | HeadAndBody HornBodyHeadClause
  deriving stock (Eq, Show, Generic)
