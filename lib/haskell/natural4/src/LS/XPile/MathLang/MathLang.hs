...
runToMathLang' ::
  SymTab VarsAndBody -> ToMathLang a -> Either ToMathLangError (a, MyState)
runToMathLang' r =
  unToMathLang
    >>> runWriterLocal
    >>> runReader r
...
getUserFuns ::
  Int -> SymTab VarsAndBody -> SymTab ([GML.Var], GML.Exp) -> SymTab VarsAndBody
