...
#+begin_src bnfc :tangle L4BNFC/natural4.bnfc
...
#+begin_src text
  ...
  IF foo IS bar                    :- bar(foo).

  IF  foo IS bar                   :- bar(foo), quux(baz).
  AND baz IS quux

  IF foo IS bar AND baz            :- bar(foo), baz(foo).
#+end_src

| L4                    | prolog                  | internally                                                      | thoughts |
|-----------------------+-------------------------+-----------------------------------------------------------------+----------|
| IF foo                | foo                     | Leaf $ RPMT (MTT "foo")                                         |          |
|-----------------------+-------------------------+-----------------------------------------------------------------+----------|
| IF foo IS bar         | bar(foo)                | Leaf $ RPConstraint (MTT "foo") RPis (MTT "bar")                |          |
| IF Color IS red       | red(Color)              |                                                                 |          |
|-----------------------+-------------------------+-----------------------------------------------------------------+----------|
| IF  foo IS bar        | bar(foo), baz           | All [ Leaf $ RPConstraint (MTT "foo") RPis (MTT "bar")          |          |
| AND baz               |                         | .   , Leaf $ RPMT (MTT "baz") ]                                 |          |
|-----------------------+-------------------------+-----------------------------------------------------------------+----------|
| IF  foo IS bar        | bar(foo), quux(baz)     | All [ Leaf $ RPConstraint (MTT "foo") RPis (MTT "bar")          |          |
| AND baz IS quux       |                         | .   , Leaf $ RPConstraint (MTT "baz") RPis (MTT "quux") ]       |          |
|-----------------------+-------------------------+-----------------------------------------------------------------+----------|
| IF foo IS bar AND baz | bar(foo), baz(foo)      | All [ Leaf $ RPConstraint (MTT "foo") RPis (MTT "bar")          |          |
|                       |                         | .   , Leaf $ RPConstraint (MTT "foo") RPis (MTT "baz")          |          |
|-----------------------+-------------------------+-----------------------------------------------------------------+----------|
| IF foo IS     bar     | bar(foo), baz(foo)      | as above                                                        |          |
| .         AND baz     |                         |                                                                 |          |
|-----------------------+-------------------------+-----------------------------------------------------------------+----------|
| IF foo IS     bar     | bar(foo), ( baz(foo)    | All [ Any [ Leaf $ RPConstraint (MTT "foo") RPis (MTT "bar")    |          |
| .         AND baz     | .         ; quux(foo) ) | .         , Leaf $ RPConstraint (MTT "foo") RPis (MTT "baz") ]  |          |
| .          OR quux    |                         | .   , Leaf       $ RPConstraint (MTT "foo") RPis (MTT "quux") ] |          |
|-----------------------+-------------------------+-----------------------------------------------------------------+----------|

but this begins to be problematic.

Now, so far, we have been imprecise about the types in the RelationalPredicate. Actually the parameters are lists of MTExprs:

| IF foo IS     bar         | bar(foo)        | All [ Leaf $ RPConstraint [MTT "foo"]            RPis [MTT "bar"]    |
| .         AND baz IS quux | , quux(foo.baz) | .   , Leaf $ RPConstraint [MTT "foo", MTT "bar"] RPis [MTT "quux"] ] |
|                           |                 |                                                                      |

But, the semantics are that baz is an attribute of foo.

So we really want to say something like

#+begin_src haskell
  RPConstraint [MTT "foo", MTT ".", MTT "bar"] RPis [MTT "quux"]
#+end_src

or perhaps

#+begin_src haskell
  RPConstraint [MTT "foo's", MTT "bar"] RPis [MTT "quux"]
#+end_src

In more detailed Prolog dict syntax this is actually something more like

#+begin_src prolog
  bar(foo), foo{baz: X}, quux(X)
#+end_src

We infer that foo has some (row-polymorphic) type

#+begin_src haskell
  newtype Foo = Foo { baz :: Maybe R }
#+end_src

The extensibility model is not fully thought out, I need help with this. Maybe we use GADT syntax. Assuming everything that we can valuate is in the ~Valuable~ typeclass:
...
We can't just mechanically transform in a purely recursive fashion. Instead we have to pay attention to the parent when constructing the BoolStrct! The implementation in the parser remains [TODO].

Maybe the transformation is as simple as promoting the inner BoolStructR up one level and prepending the parent class scope into the LHS of all RPConstraints (and maybe other RelationalPredicates).

Indeed, the work with the record hierarchy of ~foo's bar~ attribute management also remains to be implemented. Maybe we can get this done on a future Monday.

*** Other Key Questions

**** How do we disambiguate literals from predicates?

e.g.

~IF foo IS literalString~

~IF foo IS 500~

~IF foo IS predicate~

suggestion: look up the symbol table to see if ~predicate~ is defined as the head of a ~DECIDE~ or ~MEANS~

| if Alice is the child of Bob and Carol | child(alice, bob) AND child(alice,carol)          | IF,alice,IS,child,OF,bob,AND,carol   |
|                                        | child(alice, 'bob_and_carol')                     | IF,alice,IS,child,OF,bob,and,carol   |
|                                        | child(alice, bob) OR child(alice,carol)           | IF,alice,IS,child,OF,bob,UNION,carol |
|                                        | child(alice, Parents), sort([bob,carol], Parents) | IF,alice,IS,child,OF,bob,WITH,carol  |


#+begin_src haskell
  -- child(alice, bob)
  SimpleRule { nameOfSimpleRule = "about alice"
             , varDeclsOfSimpleRule = [ ]
             , precondOfSimpleRule = [ AppE { annotOfExpr = ()
                                            , funOfExprAppE = AppE { annotOfExpr = ()
                                                                   , funOfExprAppE = VarE { annotOfExpr = ()
                                                                                          , varOfExprVarE = GlobalVar { nameOfVar = QVarName { annotOfQVarName = ()
                                                                                                                                             , nameOfQVarName = "child" }
                                                                                                                      }
                                                                                          }
                                                                   , argOfExprAppE = VarE { annotOfExpr = ()
                                                                                          , varOfExprVarE = GlobalVar { nameOfVar = QVarName { annotOfQVarName = ()
                                                                                                                                             , nameOfQVarName = "alice" }
                                                                                                                      }
                                                                                          }
                                                                   }
                                            , argOfExprAppE = VarE { annotOfExpr = ()
             ...
             , postcondOfSimpleRule = _
             }
  
#+end_src


and what are the semantics of this anyway?
- child(alice, [bob, carol])
naively suggests that Bob and Carol loved each other very much and had a baby named Alice.

are these the same?
- child(alice, 'bob and carol')
- child(alice, 'carol and bob')


what are the values of:
- child(alice, [bob]) :: sadly, Carol dies and Bob is now a single parent.
- child(alice, [carol]) :: sadly, Bob dies and Carol is now a single parent.

suppose bob divorces carol and remarries eddie

would it be true that
- child(alice, [eddie])
- child(alice, [bob, eddie])
?

would it be true that
- child(alice, [carol, eddie])
?

transformation rule: we drop [OF]

This becomes

RPConstraint [MTT "alice"] RPis (RPConstraint [MTT "child"] RPof [MTT "bob", MTT "carol"])

which transforms according to the rules

RPConstraint X RPis RPConstraint Y RPof Zs -> Y(X, Zs)
...
#+begin_src prolog :tangle L4BNFC/bluecar.pl
...
* Process Algebras and State Transitions

** Obligation

*** If they perform the obligation:

IF-FULFILLED

*** If they fail to perform the obligation:

IF-VIOLATED

*** What if the obligation is a negative obligation?
...
*** If they do the permitted action:

**** And someone else has to respond in some way that they wouldn't have had to otherwise:

*** If they do not do the permitted action:
