...
pdpa1withUnexpandedUponInterp = defaultL4I
...
-- [TODO] some tests are broken because the L4I shouldn't be hardcoded -- it should result from running the interpreter on some input ruleset; hence populating the ruleGraph field correctly.

mustsingInterp :: BoolStructR -> [Rule] -> Interpreted
mustsingInterp variableBSR variableOrigRules = defaultL4I
    { classtable = CT Map.empty
    , scopetable = fromList
        [ ([ MTT "Qualifies" ]
           , fromList [
