...
say("** fromNode1 is a value");
say(AA.fromNode1);
say("** fromNode2 is a function");
say(AA.fromNode2("moo"));
say("** fromNode3 is a complex data structure");
code(AA.fromNode3, true);
function code(obj, asjson, style) {
    if (asjson === void 0) { asjson = false; }
    if (style === void 0) { style = "src javascript"; }
    say("#+begin_" + style);
    if (asjson) {
        say(JSON.stringify(obj, null, 2));
    }
    else {
        say(obj);
    }
...
code(AA.example1);
say("*** json");
code(JSON.stringify(AA.example1, null, 0));
say("*** encoded by purescript for consumption in JS");
code(AA.example1_encoded, true);
...
say("**** marking1_encoded");
say(AA.marking1_encoded, true);
say("**** marking1_encoded then decoded");
say(AA.marking1_decoded);
say("**** marking1_encoded then decoded then encoded again");
say(AA.marking1_recoded, true); // prove our codec works by roundtripping
...
say("#+NAME: fancyMarking");
code(fancyMarking, true);
...
var decodedFancyRule = AA.decodeItemString(fancyRules);
say("** external fancy Rule read into Purescript");
code(decodedFancyRule, true);
say("** external fancy Marking read into Purescript");
var decodedFancyMarking = AA.decodeMarking(fancyMarking);
code(fancyMarking, true);
console.log("* now we call paint() for the fancyRules, with soft");
var paintOut2 = AA.paint(AA.soft)(fancyMarking)(AA.getNLByName("example1"))(decodedFancyRule);
...
var paintsimple = AA.paint(AA.hard)(simpleMarking)(AA.getNLByName("example1"))(AA.getItemByName("example1"));
code(paintsimple);
say("** we call paint() on a fancyMarking, in soft mode");
var paintfancy = AA.paint(AA.soft)(fancyMarking)(AA.getNLByName("example1"))(decodedFancyRule);
